       ...
       for i, line1 in enumerate(lines):
        for j, line2 in enumerate(lines):
            if i < j:
                ...
                # If yes, the following steps will help you connect all the points involved here with the intersection point as edges.
...
def printOutput(vertices, edges):
    try:
        if(len(vertices) == 0): raise Exception("Not Enough Vertices.") 
        if len(vertices) != 0:
            print("V {}".format(len(vertices), flush=True))

        output_str = ""
        
        if(len(edges) == 0): raise Exception("Not Enough Edges.")
        if len(edges) != 0:
            n = len(edges)
            Edges = list(edges)
            for itr in Edges[:n - 1]:
                it = list(itr)
                output_str = output_str + "<{},{}>,".format(it[0], it[1])

            lastElement = list(Edges[len(Edges) - 1])
            output_str = output_str + "<{},{}>".format(lastElement[0], lastElement[1])
            print("E {" + output_str + "}", flush=True)
    except Exception as e:
        # Handle exceptions here
        print(f"An error occurred: {e}")
            ...
            try:      
                if (commandChecker(allText)):
                    if(allText[0] == 'add'):
                        ...
                        if(len(streetDict) == 0): 
                            raise Exception("Street Dictionary is Empty.")
                                        ...
                                        if(len(aux) == 0):
                                          aux.append(value[itr])
                                           ...
                                           if L not in AllLines:
                                              AllLines.append(L)
                        if(len(AllLines)==0):
                           raise Exception("Unable to find lines.")                      
                        vertices,edges = create_graph(AllLines)
                        if(len(vertices)==0 | len(edges)==0):
                           raise Exception("No Vertices or Edges.") 
